...
// character reviews and rating
app.post('/data/:id/comments', (req, res) => {
...
app.get('/data/:id/comments', (req, res) => {
  const id = parseInt(req.params.id);

  const characterComments = comments.filter((comment) => comment.characterId === id);


  const commentsWithUsername = characterComments.map((comment) => ({
    username: comment.username,
    text: comment.text,
    rating: comment.rating,
  }));

  res.json(commentsWithUsername);
});



app.delete('/data/:characterId/comments/:commentId', (req, res) => {
  ...
  if (comments[commentIndex].characterId !== characterId) {
    return res.status(400).json({ error: 'Comment does not belong to this character' });
  }
